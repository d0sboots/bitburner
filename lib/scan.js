...
export class ServerEntry {
  ...
  numCycleForGrowthCorrected(fraction) {
    ...
    // Shorthand vars: growthBase = b, threads = x, moneyMax = m, fraction = f
    // The formula we're dealing with is (f*m + x) * e^(b*x) = m, or
    //   (f + x/m) * e^(b*x) = 1
    ...
    //   x_next = x - y/y'
    //
    //   x_next = x - (log(f + x/m) + b*x)/(1/(f*m + x) + b)
    //   = (x/(f*m + x) + b*x - log(f + x/m) - b*x)/(1/(f*m + x) + b)
    //   = (x - (f*m + x)*log(f + x/m))/(1 + b*(f*m + x))
    //   = (x + 1/b - 1/b*(1 + b*(f*m + x)*log(f + x/m)))/(1 + b*(f*m + x))
    //   = (x + 1/b)/(1 + b*f*m + b*x) - log(f + x/m)/b
    const invMoney = 1 / s.moneyMax;
    const invBase = 1 / growthBase;
    const denomTerm = 1 + growthBase * fraction * s.moneyMax;
    // We use an initial guess of 1000 for x.
    // It is better to guess too high than too low, since the high side is
    // more linear. We throw in a min to protect against negative values on
    // the 2nd iteration; as long as guess doesn't exceed e*moneyMax, we can
    // never go negative.
    let guess = Math.min(s.moneyMax, 1000);
    // Now we iterate until it converges well enough.
    while (true) {
      const nextGuess = (guess + invBase) / (denomTerm + growthBase * guess) - Math.log(fraction + guess * invMoney) * invBase;
