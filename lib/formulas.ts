...
//
// Implementations are directly copied from the source. Current as of f4c4b17 (2026-04-30)
 ...
 */
export function createCurrentFormulas(ns: NS): Formulas {
  ns.getResetInfo;
 ...
 * (ns.getResetInfo() + ns.getServerRequiredHackingLevel()).
 */
export function createCurrentFormulasNoRam(ns: NS): Formulas {
 ...
 * RAM cost: 0.1GB (ns.getServerRequiredHackingLevel())
 */
export function getBn12SfLevelNoRam(ns: NS): number {
...
export class BitNodeMultipliers {
  ...
  constructor(a: Partial<Record<keyof BitNodeMultipliers, number>> = {}) {
    for (const [key, value] of Object.entries(a)) this[key as keyof BitNodeMultipliers] = clampNumber(value);
  }
}
 ...
 */
export function getBitnodeMultipliers(bn: number, sf: number): BitNodeMultipliers {
...
class Reputation {
  ...
  calculateFavorToRep(_favor: unknown): number {
    const f = helpers.number("favor", _favor);
  ...
  calculateRepToFavor(_rep: unknown): number {
    const r = helpers.number("rep", _rep);
  ...
  repFromDonation(_amount: unknown, _player: unknown): number {
    const amt = helpers.number("amount", _amount);
    const person = helpers.person(_player);
    return (amt / CONSTANTS.DonateMoneyToRepDivisor) * person.mults.faction_rep * this.bnMults.FactionWorkRepGain;
  }

  donationForRep(_reputation: unknown, _player: unknown): number {
    const rep = helpers.number("reputation", _reputation);
    const person = helpers.person(_player);
    return (rep * CONSTANTS.DonateMoneyToRepDivisor) / person.mults.faction_rep / this.bnMults.FactionWorkRepGain;
  }

  sharePower(_threads: unknown, _cpuCores: unknown = 1): number {
    const shareThreads = helpers.positiveInteger("threads", _threads);
    const cpuCores = helpers.positiveInteger("cpuCores", _cpuCores);
...
class Skills {
  ...
  calculateSkill(_exp: unknown, _mult: unknown = 1) {
  ...
  calculateExp(_skill: unknown, _mult: unknown = 1) {
    ...
    if (skill === floorSkill && Number.isFinite(skill) && Number.isFinite(value)) {
      ...
      let calcSkill = this.calculateSkill(value, mult);
      let diff = Math.abs(value * Number.EPSILON);
      let newValue = value;
      while (calcSkill < skill) {
        newValue = value + diff;
        diff *= 2;
        calcSkill = this.calculateSkill(newValue, mult);
      }
      value = newValue;
    }
...
class Hacking {
   ...
   * @param {BitNodeMultipliers} bnMults
   */
  constructor(bnMults: BitNodeMultipliers, extra: Extra) {
    this.bnMults = bnMults;
    this.extra = extra;
  }

  hackChance(_server: unknown, _player: unknown): number {
  ...
  hackExp(_server: unknown, _player: unknown): number {
    const server = helpers.server(_server);
    const person = helpers.person(_player);
  ...
  growPercent(_server: unknown, _threads: unknown, _player: unknown, _cores: unknown = 1): number {
    const server = helpers.server(_server);
    const person = helpers.person(_player);
  ...
  growThreads(_server: unknown, _player: unknown, _targetMoney: unknown, _cores: unknown = 1): number {
    const server = helpers.server(_server);
    const person = helpers.person(_player);
  ...
  growAmount(_server: unknown, _player: unknown, _threads: unknown, _cores: unknown = 1): number {
    const server = helpers.server(_server);
    const person = helpers.person(_player);
    const threads = helpers.number("threads", _threads);
    const cores = helpers.number("cores", _cores);
  ...
  hackTime(_server: unknown, _player: unknown): number {
    const server = helpers.server(_server);
    const person = helpers.person(_player);
    return calculateHackingTime(server, person) * 1000;
  }

  growTime(_server: unknown, _player: unknown): number {
    const server = helpers.server(_server);
    const person = helpers.person(_player);
  ...
  weakenTime(_server: unknown, _player: unknown): number {
    const server = helpers.server(_server);
    const person = helpers.person(_player);
  ...
  weakenEffect(_threads: unknown, _cores: unknown = 1): number {
    const threads = helpers.number("threads", _threads);
    const cores = helpers.number("cores", _cores);
    const coreBonus = getCoreBonus(cores);
    return ServerConstants.ServerWeakenAmount * threads * coreBonus * this.bnMults.ServerWeakenRate;
  }
}
...
class Extra {
   ...
   */
  calculatePercentMoneyHacked(server: Server, person: Person): number {
    const hackDifficulty = server.hackDifficulty ?? 100;
  ...
  calculateGrowthConstant(server: Server, p: Person, cores: number): number {
  ...
  calculateServerGrowthLog(server: Server, threads: number, p: Person, cores = 1): number {
    if (!server.serverGrowth) return -Infinity;
    const hackDifficulty = server.hackDifficulty ?? 100;
  ...
  calculateBatchThreads(options: { [x in keyof BatchThreadsOpts]: unknown }): { hack: number; grow: number } {
    ...
    if (hgw) {
      ...
      do {
        ...
        const log = log_inner < 0.0035 ? Math.log1p(log_inner) : ServerConstants.ServerMaxGrowthLog;
...
interface BatchThreadsOpts {
  batchType: "hgw" | "ghw";
  server: Server;
  person: Person;
  threadConstant: number;
...
type PositiveInteger = Integer & PositiveNumber;

// This interface is *huge*, we only include the methods we actually use.
interface NS {
...
interface ResetInfo {
  ...
  bitNodeOptions: unknown; // BitNodeOptions;
}
...
const helpers = {
  ...
  positiveInteger(argName: string, v: unknown): PositiveInteger {
    const n = this.number(argName, v);
    if (!isPositiveInteger(n)) {
      throw makeRuntimeErrorMsg(`${argName} must be a positive integer, was ${n}`, "TYPE");
    }
    return n;
  },
 ...
 */
function makeRuntimeErrorMsg(msg: string, type = "RUNTIME"): Error {
...
function userFriendlyString(v: unknown): string {
  const clip = (s: string) => {
 ...
 * @return {number}
 */
function calculateHackingTime(server: Server, person: Person): number {
...
function clampNumber(value: number, min = -Number.MAX_VALUE, max = Number.MAX_VALUE): number {
  if (isNaN(value)) {
...
const isPositiveInteger = (n: unknown): n is PositiveInteger => isInteger(n) && n > 0;

function clampInteger(value: number, min = -Number.MAX_SAFE_INTEGER, max = Number.MAX_SAFE_INTEGER): number {
  if (isNaN(value)) {
