...
class Extra {
   ...
   * @property {?number} fortify - security decrease to apply between h and g, defaults to 0
   ...
   * @return The calculated number of threads for the first operation
   *
   * This complicated function is used to solve for the number of needed hack() or grow() threads
   ...
   * optional parameter "fortify", which applies an specified amount of security decrease between
   * the hacks and grows.
   *
   ...
   * number of hack threads needed, and then you can substitute into the formula to determine the
   * grow threads needed. The weaken threads have already been given.
   *
   ...
   */
  calculateBatchThreads(options: { [x in keyof BatchThreadsOpts]: unknown }): number {
    ...
    const fortify = helpers.number("fortify", options.fortify ?? 0);
    ...
    //  - final_difficulty is the security after hacking and "fortify"
    ...
    //  final_difficulty = hackDifficulty + ServerFortifyAmount * h_threads - fortify
    ...
    //    log(1 + ServerBaseGrowthIncr/(hackDifficulty + ServerFortifyAmount * h_threads - fortify)) * c_grow * g_threads) = money
    //  1 - h_threads * (c_pmh - threadMultiplier / money) + threadConstant / money = exp(
    //    log1p((ServerBaseGrowthIncr/ServerFortifyAmount)/((hackDifficulty-fortify)/ServerFortifyAmount + h_threads)) *
    //    -c_grow * g_threads)
    //
    //  let k1 = ServerBaseGrowthIncr/ServerFortifyAmount and k2 = (hackDifficulty - fortify) / ServerFortifyAmount
    ...
    if (hgw) {
      ...
      const k2 = 500 * (server.hackDifficulty - fortify);
      const k3 = k1 + k2;
      ...
      do {
        ...
        const log = Math.log1p(k1 / (k2 + t));
        const exp = Math.expm1(c_grow * log * linear);
        const y = exp + t * t_mult - c;
        const yp = t_mult + c_grow * (exp + 1) * (threadMultiplier * log - (k1 * linear) / ((k2 + t) * (k3 + t)));
        const newt = t - y / yp;
        diff = newt - t;
        thresh = t * relError;
        t = newt;
