...
class Extra {
   ...
   * @property {?number} absError - accuracy termination condition, defaults to 1e-4
   ...
   * the number of iterations, while a smaller "absError" will increase it.
   *
  ...
  calculateBatchThreads(options: { [x in keyof BatchThreadsOpts]: unknown }): number {
    ...
    const absError = helpers.number("absError", options.absError ?? 1e-4);
